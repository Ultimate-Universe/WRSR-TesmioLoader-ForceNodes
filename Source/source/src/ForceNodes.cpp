...
DLL_EXPORT int TsmPluginInit(const TsmHost* host, TsmPluginInfo* info)
{
    ...
    ProvideServices();
    g_initialised = 1;
    H->log("ForceNodes  v1.8.0 initialised; Tesmio API 4 and WRSR 1.1.1.9 resolver ready for hook phase");
    return 0;
}
...
DLL_EXPORT int TsmPluginStart(void)
{
    ...
    g_started = 1;
    Core_SetReady(1);
    H->log("ForceNodes  v1.8.0 active; overlay, force placement, grid mode and safe removal ready");
    return 0;
}
