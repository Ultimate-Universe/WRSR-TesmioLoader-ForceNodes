...
static u8* Resolve(const Pattern& pattern, int allowDetouredVerifiedRva)
{
    ...
    const u32 verifiedRva = pattern.verifiedRva1119;
    if (verifiedRva && (usize)verifiedRva + pattern.length <= H->exeSize)
    {
        u8* configured = H->exeBase + verifiedRva;
        if (H->readablePtr(configured, pattern.length))
        {
            if (MaskMatch(configured, pattern))
            {
                H->log("ForceNodes  signature verified  %s exe+0x%X (scan matches=%d)",
                       pattern.label, verifiedRva, count);
                return configured;
            }
...
/* Evidence-backed WRSR 1.1.1.9 layout. The signature scan remains the primary
   resolver; these RVAs are only validated fallbacks for this game build. */
static const Pattern pFrame = {"main construction frame", kFrame, kFrameMask, sizeof(kFrame), 0x2F0F10u};
static const Pattern pSplit = {"real path splitter", kSplit, kSplitMask, sizeof(kSplit), 0x5446D0u};
static const Pattern pMerge = {"general path merger", kMerge, kMergeMask, sizeof(kMerge), 0x53E630u};
static const Pattern pRefresh = {"path-world refresh", kRefresh, kRefreshMask, sizeof(kRefresh), 0x5518B0u};
static const Pattern pInsert = {"24-byte path-point insert", kInsert24, kInsert24Mask, sizeof(kInsert24), 0x576260u};
static const Pattern pReserve = {"byte-vector reserve", kReserveByte, kReserveByteMask, sizeof(kReserveByte), 0x1E3C0u};
static const Pattern pRender = {"marker render queue", kRenderQueue, kRenderQueueMask, sizeof(kRenderQueue), 0x409660u};
}
