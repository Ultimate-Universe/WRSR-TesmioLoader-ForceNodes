...
"$LLVM_RC" /nologo /fo "$OUT/obj/ForceNodes.res" "$ROOT/build_support/ForceNodes.rc"
"$LLVM_RC" /nologo /fo "$OUT/obj/ForceNodes.Engine.res" "$ROOT/build_support/ForceNodes.Engine.rc"

LINK_COMMON=(
  /nologo /dll /machine:x64 /subsystem:windows,6.0 /nodefaultlib
  /dynamicbase /highentropyva /nxcompat /opt:ref /opt:icf /release /Brepro /version:1.8
)

"$LLD_LINK" "${LINK_COMMON[@]}" /entry:DllMain \
  /out:"$OUT/plugins/ForceNodes.dll" /implib:"$OUT/lib/ForceNodes.lib" \
  "$OUT/obj/Bindings.obj" "$OUT/obj/Core.obj" "$OUT/obj/Signatures.obj" "$OUT/obj/ForceNodes.obj" "$OUT/obj/ForceNodes.res" \
  "$OUT/lib/kernel32.lib" "$OUT/lib/ucrtbase.lib"

"$LLD_LINK" "${LINK_COMMON[@]}" /entry:DllMain \
  /out:"$OUT/plugins/ForceNodes.Engine.dll" /implib:"$OUT/lib/ForceNodes.Engine.lib" \
  "$OUT/obj/EngineShim.obj" "$OUT/obj/ForceNodes.Engine.res" "$OUT/lib/kernel32.lib"

cp "$ROOT/config/ForceNodes.ini" "$OUT/plugins/ForceNodes.ini"
python3 "$ROOT/tools/set_pe_checksum.py" "$OUT/plugins/ForceNodes.dll" "$OUT/plugins/ForceNodes.Engine.dll"
