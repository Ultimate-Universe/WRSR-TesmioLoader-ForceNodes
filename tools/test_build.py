...
def verify_plugin(path: Path, expected_exports: set[str], allowed_dlls: set[str], version_text: bytes) -> list[str]:
    ...
    check(pe.image_version == (1, 7), "image version is 1.7", failures)
...
PATTERNS: dict[str, tuple[bytes, str, int]] = {
    "frame": (bytes.fromhex("48 8B C4 48 89 58 20 55 56 57 41 55 41 56 48 8D A8 00 00 00 00 48 81 EC 00 00 00 00 0F 29 70 C8"), "xxxxxxxxxxxxxxxxx????xxx????xxxx", 0x2F0E70),
    "split": (bytes.fromhex("48 8B C4 44 89 40 18 48 89 48 08 55 53 56 57 41 54 41 55 41 56 41 57 48 8D 6C 24 88 48 81 EC 00 00 00 00"), "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx????", 0x544600),
    "merge": (bytes.fromhex("48 8B C4 48 89 48 08 55 41 54 41 55 41 56 41 57 48 8D 68 A8 48 81 EC 30 01 00 00 48 C7 45 E0 FE"), "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", 0x53E560),
    "refresh": (bytes.fromhex("40 53 48 83 EC 20 83 3D 00 00 00 00 02 48 8B D9 0F 8C 00 00 00 00 83 B9 58 02 00 00 00 0F 85"), "xxxxxxxx????xxxxxx????xxxxxxxxx", 0x5517E0),
    "insert24": (bytes.fromhex("4C 89 4C 24 20 4C 89 44 24 18 53 56 57 41 54 41 55 41 56 41 57 48 83 EC 50 48 C7 44 24 30 FE FF FF FF 4D 8B C8 4C 8B E2 48 8B F9 4C 8B 19 4D 2B C3 49 BD AB AA AA AA AA AA AA 2A 49 8B C5 49 F7 E8 4C 8B F2 49 C1 FE 02"), "x" * 72, 0x576190),
    "reserve-byte": (bytes.fromhex("48 83 EC 28 4C 8B 51 10 4C 8B C2 48 8B 51 08 49 8B C2 48 2B C2 4C 8B C9 49 3B C0"), "x" * 27, 0x1E3C0),
    "render": (bytes.fromhex("48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 48 81 EC 00 00 00 00 80 3D"), "xxxxxxxxxxxxxxxxxxx????xx", 0x4095C0),
}
...
def main() -> int:
    ...
    check(ini_data.startswith(b"; ForceNodes v1.7.0"), "configuration version header is v1.7.0", failures)
