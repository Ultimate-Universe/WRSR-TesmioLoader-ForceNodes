...
set "LFLAGS=/nologo /dll /machine:x64 /subsystem:windows,6.0 /nodefaultlib /dynamicbase /highentropyva /nxcompat /opt:ref /opt:icf /release /Brepro /version:1.7 /entry:DllMain"

"%LLD_LINK%" %LFLAGS% /out:"%OUT%\plugins\ForceNodes.dll" /implib:"%OUT%\lib\ForceNodes.lib" "%OUT%\obj\Bindings.obj" "%OUT%\obj\Core.obj" "%OUT%\obj\Signatures.obj" "%OUT%\obj\ForceNodes.obj" "%OUT%\lib\kernel32.lib" "%OUT%\lib\ucrtbase.lib" || exit /b 1
"%LLD_LINK%" %LFLAGS% /out:"%OUT%\plugins\ForceNodes.Engine.dll" /implib:"%OUT%\lib\ForceNodes.Engine.lib" "%OUT%\obj\EngineShim.obj" "%OUT%\lib\kernel32.lib" || exit /b 1
...
echo.
echo Built ForceNodes v1.7.0 in "%OUT%\plugins"
