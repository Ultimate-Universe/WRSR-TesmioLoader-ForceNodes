...
The commands may be overridden with `CLANG_CL`, `LLD_LINK`, and `LLVM_RC` environment variables.

## Zig alternative

Zig 0.14 or later can compile, resource-compile, and link the same native x64 Windows targets without a Windows SDK:

```bash
./build-zig.sh
```

Set `ZIG` to select a non-default executable. The Zig build also runs the same PE verifier and writes the same three runtime files.
...
- compiled Windows file/product version resources;
- SHA-256 hashes.
