...
Experimental service versions 4 and 5 are intentionally not published. Native path-preview transaction hooks are disabled in v1.8.0, so the capability mask is zero and preview-only transaction methods return failure without changing game state.
